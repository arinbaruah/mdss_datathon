...
df_waste_state <- df_waste %>% filter(stream == "Total") %>%  group_by(jurisdiction,type) %>%
  summarise(Total_tonnes = sum(tonnes)) %>% filter(jurisdiction !="Australia")

pl1 <- treemap(df_waste_state,
            index=c("jurisdiction","type"),
            vSize="Total_tonnes",
        type = "index",title = "Waste generated among each states of Australia (2006-2021)") 
